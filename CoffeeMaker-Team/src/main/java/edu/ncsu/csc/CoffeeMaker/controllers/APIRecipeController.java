...
public class APIRecipeController extends APIController {
     ...
     *
     * @param recipe
     *            the valid Recipe with which to update the existing recipe.
     * @return ResponseEntity indicating success if the Recipe could be updated
     *         or an error if it could not be
     */
    @PutMapping ( BASE_PATH + "/recipeEdit" )
    public ResponseEntity editRecipe ( @RequestBody final Recipe recipe ) {
        final Recipe foundRecipe = service.findByName( recipe.getName() );
        if ( null != foundRecipe ) {
            foundRecipe.setPrice( recipe.getPrice() );
            foundRecipe.setIngredients( recipe.getIngredients() );
            service.save( foundRecipe );
            return new ResponseEntity( successResponse( recipe.getName() + " successfully saved." ), HttpStatus.OK );
        }
        else {
            return new ResponseEntity( errorResponse( "Edits could not be saved. Recipe with the name "
                    + recipe.getName() + " could not be found in the system." ), HttpStatus.NOT_FOUND );
        }
    }
